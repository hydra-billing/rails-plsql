module ActiveRecord::PLPGSQL
  class SetOfRelation < ActiveRecord::Relation
    include ActiveRecord::PLPGSQL::SetOf::ClassMethods
    ...
    def where(opts, *rest)
      return super unless @klass.set_of? && set_of_arguments.any?

      set_of_args = set_of_arguments_names.map(&:to_sym)
      normalized_opts = normalize_arguments_conditions(opts, set_of_args)
      return super if normalized_opts.empty?

      set_of_binds = get_set_of_arguments(table_binds, normalized_opts)
      where_opts = normalized_opts.reject { |k| set_of_args.include?(k) }

      rel = spawn.from!(
        table_name_with_arguments,
        set_of_function_alias.to_sym,
        set_of_binds
      )
    ...
    def get_set_of_arguments(current, values)
      if values.is_a?(Hash)
        set_of_arguments_names.map do |name|
    ...
    def exec_queries
      return super unless @klass.set_of? && !set_of_arguments.empty?
    ...
    def normalize_arguments_conditions(opts, args)
      case opts
      when Hash
        if opts.key?(klass.set_of_function_name)
          opts[klass.set_of_function_name].symbolize_keys
        else
          opts.symbolize_keys
        end
